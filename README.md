...
- **급여 계산**: 세전 급여 + 상여금/성과급(선택) + 부양가족 수(+ 8~20세 자녀 수) 입력 → 4대보험·소득세·지방소득세 공제 후 실수령액 계산. 상여금은 같은 달 세전 급여와 합산된 금액을 기준으로 4대보험료·소득세를 계산한다. 소득세는 국세청 근로소득 간이세액표(2026.03.01. 기준) 실제 데이터로 조회
- **CSV 일괄 업로드**: 여러 직원의 급여를 한 번에 업로드해 일괄 계산. 잘못된 행(값 누락·비숫자·음수)은 건너뛰고, 어느 행이 왜 실패했는지 별도로 안내. 업로드 형식을 직접 맞출 필요 없이, 예시 행이 채워진 CSV 템플릿을 바로 다운로드할 수 있음
...
- **급여명세서 PDF**: 계산 이력 각 건을 급여명세서 형태의 PDF로 다운로드. 여러 건을 한 번에 받고 싶을 때는 현재 적용된 필터(또는 전체 이력) 기준으로 ZIP 일괄 다운로드도 가능
- **월별/연도별/직원별 집계**: 계산 이력을 월·연도·직원 단위로 묶어 총 지급액·총 공제액·평균 실수령액 등을 집계하고, 월별 추이를 꺾은선 그래프로 표시. 이력 표는 직원 선택 드롭다운으로도 필터링 가능
- **인증**: 회원가입/로그인(JWT), 비밀번호 변경. 로그인 상태를 잊었을 때는 가입 시 등록한 보안 질문/답변으로 본인 확인 후 비밀번호 재설정 가능. 계산 이력은 사용자별로 분리되어 본인 이력만 조회·수정·삭제 가능. 같은 계정으로 로그인에 5회 연속 실패하면 15분간 계정이 잠겨(비밀번호가 맞아도 로그인 불가), IP 기준 요청 속도 제한과는 별개로 여러 IP에 걸친 무차별 대입 시도도 방어
